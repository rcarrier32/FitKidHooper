...
/** Core messages UI — use embedded in Squad tab or inside the sheet overlay. */
export function MessagesPanel({
  ...
  const [active, setActive] = useState(initialFriend);
  ...
  const header = active
    ? (
      <>
        <button
          type="button"
          onClick={() => { setActive(null); loadInbox(); }}
          style={{ background: "none", border: "none", color: P, fontSize: 18, cursor: "pointer", padding: "0 8px 0 0" }}
        >
          ‹
        </button>
        {active.name || "Friend"}
  ...
  return (
    <div style={shellStyle}>
      <div style={{
        ...
        padding: embedded ? "12px 14px" : "14px 18px",
        borderBottom: `1px solid ${bd}`, flexShrink: 0,
      }}>
        <span style={{ fontSize: embedded ? 14 : 15, fontWeight: 800, color: "var(--fkh-text)", display: "flex", alignItems: "center" }}>
          {header}
        </span>
        {!embedded && onClose && (
          <button type="button" onClick={() => { onUnreadChange?.(); onClose(); }} aria-label="Close"
      ...
      {!active ? (
        <div style={{ flex: 1, overflowY: "auto", padding: "6px 0" }}>
          {threads === null ? (
            ...
            <button
              key={t.other_id}
              type="button"
              onClick={() => setActive({ id: t.other_id, name: t.name || (t.username ? `@${t.username}` : "Friend") })}
              style={{
                width: "100%", display: "flex", alignItems: "center", gap: 12,
                padding: embedded ? "12px 14px" : "12px 18px",
                background: "transparent", border: "none", borderBottom: `1px solid ${bd}`,
                cursor: "pointer", textAlign: "left",
              }}
            >
              <div style={{
                width: 38, height: 38, borderRadius: "50%", background: `${P}22`,
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                ...
                {t.unread > 0 && (
                  <span style={{ fontSize: 10, fontWeight: 800, color: "#000", background: P, borderRadius: 99, padding: "1px 7px" }}>
                    {t.unread}
                  </span>
                )}
              </div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: embedded ? "12px 14px" : "14px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
            {loadError && (
            ...
            {msgs.map(m => (
              <div
                key={m.id}
                style={{
                  alignSelf: m.from_me ? "flex-end" : "flex-start", maxWidth: "78%",
                  background: m.from_me ? P : "rgba(255,255,255,0.06)",
                  color: m.from_me ? "#000" : "var(--fkh-text)",
                  borderRadius: m.from_me ? "14px 14px 4px 14px" : "14px 14px 14px 4px",
          ...
          <div style={{
            display: "flex", gap: 8, padding: embedded ? "10px 12px" : "10px 14px",
            borderTop: `1px solid ${bd}`,
            paddingBottom: embedded ? "10px" : "calc(10px + env(safe-area-inset-bottom, 0px))",
          }}>
            <input
              value={draft}
              ...
              placeholder="Message…"
              style={{
                flex: 1, padding: "10px 12px", borderRadius: 99, border: `1px solid ${bd}`,
...
export default function MessagesSheet(props) {
  const handleClose = () => {
  ...
  return (
    <div
      onClick={handleClose}
      style={{
        position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", zIndex: 320,
        display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(6px)",
      }}
    >
      <div onClick={e => e.stopPropagation()}>
        <MessagesPanel {...props} onClose={handleClose} />
      </div>
    </div>
  );
}
